...
const RecommendationSection = () => {
    ...
    const interestOptions = ["Seni & Budaya", "Sejarah", "Teknologi", "Edukasi", "Seni Lukis", "Budaya Jawa", "Arsitektur", "Sains"];
    ...
    const museums = [
        {
            ...
            image: "https://casaindonesia.com/lkgallery/teaser/ullen_teaser_46_201901171654348z7kfe.jpg",
            rating: 4.8,
        ...
        <section className="py-16 bg-gradient-to-br from-amber-50 to-yellow-100">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-amber-700 text-lg font-body max-w-3xl mx-auto">Dapatkan rekomendasi museum yang sesuai dengan minat dan preferensi Anda</p>
                </div>

                <div className="grid lg:grid-cols-2 gap-12">
                    {/* Preference Form */}
                    <div className="bg-white rounded-2xl shadow-lg p-8">
                        <h3 className="text-2xl font-display text-amber-900 mb-6">Ceritakan Preferensi Anda</h3>

                        {/* Interest Selection */}
                        <div className="mb-6">
                            <label className="block text-amber-700 font-heading text-sm mb-3">Minat Anda (Pilih yang sesuai):</label>
                            <div className="grid grid-cols-2 gap-2">
                                {interestOptions.map((interest) => (
                                    <button key={interest} onClick={() => handleInterestToggle(interest)} className={`px-4 py-2 rounded-lg text-sm font-body transition-all duration-300 ${userPreferences.interests.includes(interest) ? "bg-amber-500 text-amber-900 shadow-md" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                                        {interest}
                                    </button>
                                ))}
                            </div>
                        ...
                        {/* Budget */}
                        <div className="mb-6">
                            <label className="block text-amber-700 font-heading text-sm mb-3">Budget Tiket:</label>
                            <div className="grid grid-cols-3 gap-2">
                                {[
                                    ...
                                    <button key={option.value} onClick={() => handlePreferenceChange("budget", option.value)} className={`px-4 py-2 rounded-lg text-sm font-body transition-all duration-300 ${userPreferences.budget === option.value ? "bg-amber-500 text-amber-900 shadow-md" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                                        {option.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        {/* Time Available */}
                        <div className="mb-6">
                            <label className="block text-amber-700 font-heading text-sm mb-3">Waktu yang Tersedia:</label>
                            <div className="grid grid-cols-3 gap-2">
                                {[
                                    ...
                                    <button key={option.value} onClick={() => handlePreferenceChange("timeAvailable", option.value)} className={`px-4 py-2 rounded-lg text-sm font-body transition-all duration-300 ${userPreferences.timeAvailable === option.value ? "bg-amber-500 text-amber-900 shadow-md" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                                        {option.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        {/* Group Size */}
                        <div className="mb-6">
                            <label className="block text-amber-700 font-heading text-sm mb-3">Ukuran Grup:</label>
                            <div className="grid grid-cols-3 gap-2">
                                {[
                                    ...
                                    <button key={option.value} onClick={() => handlePreferenceChange("groupSize", option.value)} className={`px-4 py-2 rounded-lg text-sm font-body transition-all duration-300 ${userPreferences.groupSize === option.value ? "bg-amber-500 text-amber-900 shadow-md" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                                        {option.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        {/* Transportation */}
                        <div className="mb-8">
                            <label className="block text-amber-700 font-heading text-sm mb-3">Transportasi:</label>
                            <div className="grid grid-cols-2 gap-2">
                                {[
                                    { value: "public", label: "Transportasi Umum" },
                                    { value: "private", label: "Kendaraan Pribadi" },
                                ].map((option) => (
                                    <button key={option.value} onClick={() => handlePreferenceChange("transportation", option.value)} className={`px-4 py-2 rounded-lg text-sm font-body transition-all duration-300 ${userPreferences.transportation === option.value ? "bg-amber-500 text-amber-900 shadow-md" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                                        {option.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        <button onClick={generateRecommendations} disabled={userPreferences.interests.length === 0} className="w-full bg-gradient-to-r from-amber-500 to-yellow-500 text-amber-900 py-3 px-6 rounded-lg font-button hover:from-amber-400 hover:to-yellow-400 transition-all duration-300 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    {/* Recommendations */}
                    <div className="bg-white rounded-2xl shadow-lg p-8">
                        <h3 className="text-2xl font-display text-amber-900 mb-6">Rekomendasi untuk Anda</h3>
                        ...
                        {isLoading ? (
                                ...
                                {recommendations.map((museum, index) => (
                                    <div key={museum.id} className="border border-amber-200 rounded-lg p-4 hover:shadow-md transition-all duration-300">
                                        <div className="flex items-start space-x-4">
                                            <img src={museum.image} alt={museum.name} className="w-16 h-16 rounded-lg object-cover" />
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between mb-2">
                                                    <h4 className="font-display text-amber-900 text-lg">{museum.name}</h4>
                                                    <div className="flex items-center space-x-2">
                                                        <div className="bg-amber-100 text-amber-800 px-2 py-1 rounded-full text-xs font-button">#{index + 1}</div>
                                                        <div className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-button">{museum.matchScore}% Match</div>
                                                    </div>
                                                </div>
                                                <p className="text-amber-600 font-body text-sm mb-2">
                                                    📍 {museum.location} • ⏱️ {museum.duration} • 💰 Rp {museum.price.toLocaleString()}
                                                </p>
                                                <p className="text-amber-700 font-body text-sm mb-3">{museum.description}</p>
                            ...
                            <div className="text-center py-12">
                                ...
                                <h4 className="text-xl font-display text-amber-900 mb-2">Belum Ada Rekomendasi</h4>
                                <p className="text-amber-600 font-body">Pilih minat Anda dan klik "Dapatkan Rekomendasi" untuk melihat museum yang sesuai</p>
                            </div>
                        )}
                    </div>
                </div>
